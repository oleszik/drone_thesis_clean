...
class Quad15DEnv(gym.Env):
    ...
    def __init__(
        ...
        act_high = np.ones((4,), dtype=np.float32)
        ...
        self.last_action_norm = np.zeros(4, dtype=np.float32)
        self.step_count = 0
    ...
    def step(self, action):
        ...
        raw_action = raw_action[:4] if raw_action.size >= 4 else np.pad(raw_action, (0, max(0, 4 - raw_action.size)))
        ...
        task_key = (self.task.name or "").strip().lower()
        action_phys = action_norm * self._action_physical_limits()
