...
from scripts.bridge.cli import parse_args
from scripts.bridge.config import build_run_paths
from scripts.bridge.coverage import CoverageGridTracker
from scripts.bridge.mavlink_client import connect_mavlink, maybe_switch_to_sitl_master
from scripts.bridge.model_select import resolve_runtime_model, resolve_sitl_recommended_profile, scan_path_scale_upper
from scripts.bridge.pathing import (
    auto_accept_radius,
    auto_step_len,
    auto_vxy_cap,
    bounds_minmax_xy,
    build_lawnmower_targets,
    clamp_target_xy_minmax,
    scan_center_offset_for_reference,
)
from scripts.bridge.reporting import print_summary_paths, print_summary_payload, write_summary
from scripts.bridge.tracking import clamp_norm_xy, compute_next_target, unit_xy
...
def main() -> None:
    ...
    selected_profile, model_path = resolve_runtime_model(
        task_key=task_key,
        model_arg=str(args.model),
        cfg=cfg,
        ctx="ardupilot_bridge:auto",
    )

    scale_upper = float(scan_path_scale_upper(cfg))
    ...
    if sitl_enabled:
        sitl_recommended_arm, winner_cfg, sitl_recommended_source_used, sitl_recommended_profile_match = (
            resolve_sitl_recommended_profile(sitl_src_raw, scale_upper)
        )
        applied: list[str] = []
    ...
    step_len = float(args.step_len_m) if args.step_len_m is not None else auto_step_len(scale_upper)
    accept_radius = float(args.accept_radius_m) if args.accept_radius_m is not None else auto_accept_radius(scale_upper)
    base_vxy_cap = float(args.vxy_cap) if args.vxy_cap is not None else auto_vxy_cap(scale_upper)
    ...
    connection_used = maybe_switch_to_sitl_master(
        connection=connection_requested,
        prefer_sitl_tcp=int(args.prefer_sitl_tcp),
        probe_timeout_s=1.0,
    )
    run_paths = build_run_paths(selected_profile=selected_profile)
    ...
    targets = build_lawnmower_targets(
        bounds_w,
        bounds_h,
        float(args.margin_m),
        dense_step_len,
    ...
    if int(args.dry_run) == 0:
        mav, mavutil = connect_mavlink(connection_used)
        try:
    ...
    with telemetry_path.open("w", encoding="utf-8") as telem_f:
        while str(preflight_result.get("status", "")) == "ok":
            ...
            if should_refresh and target_idx < (len(targets) - 1) and int(policy_tick_flag) == 1:
                ...
                dir_unit = unit_xy(dir_hint, default_xy=policy_action)
    ...
    print_summary_paths(telemetry_path, summary_path)
    print_summary_payload(summary)
